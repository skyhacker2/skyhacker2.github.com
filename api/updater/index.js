...
function updateApkInfoConfig(infos, existsConfig) {
  ...
  if (existsConfig.versionCode2 >= first.versionCode) {
    console.log("versionCode比目前的小 " + first.versionCode + " " + existsConfig.versionCode2);
    return existsConfig;
  }

  existsConfig.versionCode2 = first.versionCode;
  existsConfig.versionName2 = first.versionName;
  existsConfig.channels2 = {};

  for (var i = 0; i < infos.length; i++) {
    var channel = infos[i].channel;
    existsConfig.channels2[channel] = "";
  }
...
function uploadApks(remoteFolder, appName, apksDir, apkNames, current, config, callback) {
  ...
  if (apkName.endsWith("apk")) {
    ...
    uploadFileToServer(apkPath, remoteFolder, function (err, ret) {
      if (!err) {
        console.log("上传" + apkName + "成功!");
        config.channels2[channelMaps[apkName]] = "http://d.apptor.me/" + remoteFolder + apkName;
        if (fs.existsSync(apkPath)) {
          fs.unlinkSync(apkPath);
        }
...
function processApps(root, lists, current, callback) {
  ...
  if (stats.isDirectory()) {
    ...
    if (fs.existsSync(apksDir)) {
      ...
      var versionCode = existsConfig.versionCode2;
